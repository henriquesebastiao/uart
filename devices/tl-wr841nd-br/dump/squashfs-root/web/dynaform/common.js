...
var standardRouterMode = 1;/*׼·ģʽ*/
var normal3gMode = 2;/*3G·ģʽ*/
var apcMode = 4;/*AP Clientģʽ*/
var apMode = 8;/*APģʽ*/
var pure3gMode = 16;/*3Gģʽ*/
...
function showTipMsg(isMultiSysMode, sysMode, accessType, isDualBand, labelId, stepId)
{
	...
	for (i = startIdx; i <= endIdx; i++)
	{
		...
		switch (i)
		{
		case sysModeIdx:/*ǶģʽƷȥһ*/
			if (isMultiSysMode != 1)
			{
				nextLoop = 1;
			}
			break;
		case netInIdx:/*Ǳ׼3G·ȥһ*/
			if (sysMode != normal3gMode)
			{
				nextLoop = 1;
			}
			break;
		case wanOr3gIdx:/*Ǳ׼3G·߳һʱҲȥһ*/
			...
			break;
		case wanTypeIdx:
			if (sysMode != standardRouterMode /*APʹ3G·ȥһ*/
				&& sysMode != normal3gMode
				&& sysMode != apcMode)
			{
				nextLoop = 1;
			}
			if (sysMode == normal3gMode && /*3G onlyȥһ*/
			...
			break;
		case net3gIdx:/**/
			if (sysMode != normal3gMode/*APʹ3G·ȥһ*/
				&& sysMode != pure3gMode)
			{
				nextLoop = 1;
			}
			...
			break;
		case wlanModeIdx:/*APȥһ*/
			if (sysMode != apMode)
			{
				nextLoop = 1;
			}
			break;
		case wlanBandIdx:/*˫Ƶ·ȥ*/
		case wlan2gIdx:
		case wlan5gIdx:
			if (isDualBand != 1)
			{
